...
*/
	...
	//center of view
	_ctr.x = (int)((GLfloat)glutGet(GLUT_WINDOW_WIDTH) * 0.5f);
	_ctr.y = (int)((GLfloat)glutGet(GLUT_WINDOW_HEIGHT) * 0.5f);
...
void SimpleTrackball::startDolly(int x, int y)
{
  _startPt[1] = y;
  _dolly = true;
}

void SimpleTrackball::stopDolly(int x, int y)
{
  _trans[2] += _transTemp[2];
  _transTemp[2] = 0.f;
  _dolly = false;
}


void SimpleTrackball::startTranslate(int x, int y)
{
  _startPt[0] = x;
  _startPt[1] = y;
  _translate = true;
...
void SimpleTrackball::stopTranslate(int x, int y)
{
  for(int i=0;i<3;i++){
    _trans[i] += _transTemp[i];
    _transTemp[i]=0.f;
  }
  _translate = false;
}
...
void SimpleTrackball::rollBall(int x, int y)
{
  ...
  if(_drag){
    ...
    _transTemp[2] = ((y-_startPt[1]) / (float)glutGet(GLUT_WINDOW_HEIGHT)) * _translateSpeed;
  }
  else if(_translate){
    _transTemp[0] = ((x-_startPt[0]) / (float)glutGet(GLUT_WINDOW_WIDTH)) * _translateSpeed;
    _transTemp[1] = ((y-_startPt[1]) / (float)glutGet(GLUT_WINDOW_HEIGHT)) * _translateSpeed;
  }
...
void SimpleTrackball::multModelMatrix(){
  glTranslatef(_trans[0]+_transTemp[0], _trans[1]+_transTemp[1], _trans[2]+_transTemp[2]);
